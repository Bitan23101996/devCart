...
router.get("/", (req, res) => {
  ...
  const products = addImageUrls(req, results);
  res
    .status(200)
    .json(
      response.generate(false, "Product(s) fetched succesfully", 200, products),
    );
});
...
router.post("/", (req, res) => {
  ...
  if (category && price && name && description) {
    ...
    res
      .status(201)
      .json(
        response.generate(false, "Product(s) added successfully", 201, data),
      );
  } else {
...
router.get("/:productId", (req, res) => {
  ...
  if (productId && typeof +productId === "number") {
    ...
    const productResponse = addImageUrl(req, product);
    ...
    if (product) {
      ...
      res
        .status(200)
        .json(response.generate(false, "No product found", 200, null));
    }
  } else {
    res.status(400).json(response.generate(true, "Bad request", 400, null));
  }

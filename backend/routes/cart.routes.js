...
const { addImageUrl, addImageUrls } = require("../utils/image.utils");
...
router.post("/", (req, res) => {
  ...
  if (username) {
    ...
    const results = addImageUrls(
      req,
      db.cart.filter((p) => p.username === username),
    );
  ...
  return res
    .status(400)
    .json(response.generate(true, "Invalid input", 400, null));
});
...
router.post("/addCart", (req, res) => {
  ...
  if (username && productId && typeof productId === "number") {
    const db = readDB();
    const availableProduct = db.cart.find(
      (p) => p.id === +productId && p.username === username,
    );
    if (availableProduct) {
      ...
      return res
        .status(200)
        .json(
          response.generate(false, "Cart updated successfully", 200, userCart),
        );
    }
    ...
    if (results) {
      ...
      return res
        .status(200)
        .json(
          response.generate(false, "Cart added successfully", 200, userCart),
        );
    } else {
      // res.status(400).json({ message: 'No product found', ...req.body });
      return res
        .status(400)
        .json(response.generate(false, "No product found", 400, null));
    }
  }
});
...
router.post("/deleteCart", (req, res) => {
  ...
  if (username && productId && typeof productId === "number") {
    ...
    if (availableProduct && availableProduct.quantity > 1) {
      ...
      res
        ...
        .json(
          response.generate(
            false,
            "Item from cart reduced successfully",
            200,
            userCart,
          ),
        );
    ...
    const results = db.cart.filter(
      (p) =>
        !(p.id === +productId && p.quantity === 1 && p.username === username),
    );
    db.cart = results;
    writeDB(db);
...
router.post("/clearCart", (req, res) => {
  ...
  if (!username) {
    return res
      .status(400)
      .json(response.generate(true, "Username is required", 400, null));
  }
  ...
  if (userCart.length === 0) {
    return res
      .status(404)
      .json(response.generate(true, "Cart is already empty", 404, null));
  }
  ...
  return res
    .status(200)
    .json(response.generate(false, "Cart cleared successfully", 200, []));
});
